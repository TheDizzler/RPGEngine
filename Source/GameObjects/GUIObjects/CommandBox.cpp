...
// read reply attribute of selected
	const char_t* reply = nodeList[itemSelected].attribute("reply").as_string();
	...
	for (xml_node child = parent.child("dialogReply"); child; child = child.next_sibling("dialogReply")) {
		string att = child.attribute("reply").as_string();
		if (att == reply_s)
			return child;
	}
}
